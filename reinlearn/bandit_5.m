function bandit_5(n, var, steps)
%% 本程序模拟多臂赌博机的工作情形，并研究在constant average方法下用策略下(esp-greedy)获得最大报酬的规律
...
% exr: 在esp-greedy策略下获得的‘行动轮数-报酬’向量
    ...
    rmean = rmean/500;
    plot(rmean, 'r'); gtext("esp = 0.1");hold on
...
% 模拟一次game全过程，输出报酬变化向量和行动向量
    ...
    for t = 1:steps
        ...
        if cnta(at(t)) == 1
            ...
            qaction(at(t)) =  qaction(at(t)) + (rv(t) - qaction(at(t))) * 0.1;
        end
        if t == 1
            q = rv(t);
        else

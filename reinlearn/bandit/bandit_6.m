...
% 模拟一次game全过程，输出报酬变化向量和行动向量
    ...
    for t = 1:steps
        ...
        sumr(at(t, 1)) = sumr(at(t, 1)) + rv(t, 1); 
        cnta(at(t, 1), 1) = cnta(at(t, 1), 1) + 1;
        qaction(at(t, 1), 1) =  sumr(at(t, 1)) / cnta(at(t, 1));
        q(1) = q(1) + (rv(t, 1) - q(1)) /t;
        rmean(t, 1) = q(1);
        ...
        if cnta(at(t, 2), 2) == 1
            ...
            qaction(at(t, 2), 2) =  qaction(at(t, 2), 2) + (rv(t, 2) - qaction(at(t, 2), 2)) * 0.1;
        end
        if t == 1
            q(2) = rv(t);
        else
            q(2) = q(2) + (rv(t, 2) - q(2)) * 0.1;
        end
 %       q(2) = q(2) + (rv(t, 2) - q(2)) /t;
        rmean(t, 2) = q(2);
    end
end